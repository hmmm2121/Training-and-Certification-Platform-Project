...
namespace TrainingAndCertificationPlatform.Controllers
{
    ...
    public class RoomsController : Controller
    {
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (room != null)
            {
                // check if room is used in any sessions
                if (_context.CourseSessions.Any(cs => cs.RoomId == id))
                {
                    TempData["ErrorMessage"] = "Cannot delete this room as it is assigned to existing sessions.";
                    return RedirectToAction(nameof(Index));
                }

                // remove equipment first then room
